...
def classifyEmotions(net, inputDirectory, outputFile, landmarksFile):
    ...
    while (counter < len(imagesList)):
        ...
        for index in range(0, sizeOfRange):
            image = cv2.imread(os.path.join(inputDirectory, imagesList[index + counter]), 1)
        ...
        for index in range(0, sizeOfRange):
            genderLabels = out['reshape'][index][:, 0][:2]
            ageLabels = out['reshape'][index][:, 1]
            genderLabels = np.resize(genderLabels/np.sum(genderLabels), len(genderLabels))
            ageLabels = np.resize(ageLabels/np.sum(ageLabels), len(ageLabels))
            labels = landmarks[index + counter].split()
            f.write("{} {} {} {}\n".format(labels[0], labels[1],' '.join(map(str, genderLabels)), ' '.join(map(str, ageLabels))))
