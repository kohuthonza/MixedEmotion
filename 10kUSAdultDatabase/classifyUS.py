...
def classify(net, inputDirectory, xlsxTable, statisticXlsxTable, trainXlsxTable):
    ...
    for index in range(0, len(MSE)):
        print ('{} {} {}%'.format(USTable.columns[index], MSE[index], PercentageDif[index]*100))
        statisticTable.loc[USTable.columns[index]][statisticIndex*5] = PercentageDif[index]*100
        statisticTable.loc[USTable.columns[index]][statisticIndex*5 + 1] = MSE[index]
        statisticTable.loc[USTable.columns[index]][statisticIndex*5 + 2] = RMSE[index]
        statisticTable.loc[USTable.columns[index]][statisticIndex*5 + 3] = MSE[index]/MeanMSE[index]
        statisticTable.loc[USTable.columns[index]][statisticIndex*5 + 4] = RMSE[index]/MeanRMSE[index]
